...
const threeSceneControlsDisabled = computed(() => {
  return isThreeStageModelRenderer(stageModelRenderer.value) && vrmSceneMutationLocked.value
})
...
const viewControlsValueX = computed({
  ...
  set: (value) => {
    switch (stageModelRenderer.value) {
      ...
      case 'vrm':
      case 'mmd':
        if (threeSceneControlsDisabled.value)
          break
...
const viewControlsValueScale = computed({
  ...
  set: (value) => {
    ...
    else if (!threeSceneControlsDisabled.value) {
...
<template>
  <Transition name="fade-side-pops-in">
    <div v-if="stageViewControlsEnabled" :class="threeSceneControlsDisabled ? ['opacity-60', 'pointer-events-none'] : []">
      <Transition name="fade-side-pops-in" mode="out-in">
        <div v-if="props.mode === 'x'" relative class="[&_.round-range-tooltip]:hover:opacity-100">
          <RoundRange v-model="viewControlsValueX" :min="viewControlsValueXMin" :max="viewControlsValueXMax" :step="0.01" :disabled="threeSceneControlsDisabled" data-direction="vertical" h="50%" write-vertical-left />
        ...
        <div v-else-if="props.mode === 'y'" relative class="[&_.round-range-tooltip]:hover:opacity-100">
          <RoundRange v-model="viewControlsValueY" :min="viewControlsValueYMin" :max="viewControlsValueYMax" :step="0.01" :disabled="threeSceneControlsDisabled" write-vertical-left h="50%" data-direction="vertical" />
          <div class="round-range-tooltip" top="50%" translate-y="[-50%]" absolute left-10 font-mono op-0 transition="all duration-200 ease-in-out">
            {{ viewControlsValueY.toFixed(2) }}
          </div>
        </div>
        <div v-else-if="isThreeStageModelRenderer(stageModelRenderer) && props.mode === 'z'" relative class="[&_.round-range-tooltip]:hover:opacity-100">
          <RoundRange v-model="viewControlsValueZ" :min="viewControlsValueZMin" :max="viewControlsValueZMax" :step="0.01" :disabled="threeSceneControlsDisabled" write-vertical-left h="50%" data-direction="vertical" />
          <div class="round-range-tooltip" top="50%" translate-y="[-50%]" absolute left-10 font-mono op-0 transition="all duration-200 ease-in-out">
            {{ viewControlsValueZ.toFixed(2) }}
          </div>
        </div>
        <div v-else-if="props.mode === 'scale'" relative class="[&_.round-range-tooltip]:hover:opacity-100">
          <RoundRange v-model="viewControlsValueScale" :min="0" :max="3" :step="0.0001" :disabled="threeSceneControlsDisabled" write-vertical-left h="50%" data-direction="vertical" />
          <div class="round-range-tooltip" top="50%" translate-y="[-50%]" absolute left-10 font-mono op-0 transition="all duration-200 ease-in-out">
